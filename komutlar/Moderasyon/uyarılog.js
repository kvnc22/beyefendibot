...
// mete işin bittiyse kayıt sisteminin yardım menüsünü yazabilir misin?   yazarim tabii ki
  ...
  if(!kanal) return message.reply('<a:iptal:722904688969777202> Bu Özelliği Ayarlamam İçin Bir Kanal Etiketlemelisin Örnek: `-uyarı-log #logkanal`')
  
  message.channel.send('Uyarı log Kanalını '+kanal+' Olarak Ayarladım.')
  
  db.set(`uyarıkanal_${message.guild.id}`, kanal.id)
  };
exports.conf = {
  enabled: true,  
  guildOnly: false, 
